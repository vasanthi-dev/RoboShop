...
CREATE(){
  ...
  if [ $COUNT -eq 0 ]
   then
    ...
    aws ec2 run-instances --image-id ami-0dc863062bc04e1de --launch-template LaunchTemplateId=lt-099eb0b79a90eeba3,Version=2 --tag-specifications "ResourceType=instance,Tags=[{Key=Name,Value=$1}]" "ResourceType=spot-instances-request,Tags=[{Key=Name,Value=$1}]" | jq &>>/dev/null
  else
